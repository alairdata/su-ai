...
export default function BackofficePage() {
  ...
  useEffect(() => {
    fetch("/api/backoffice/verify")
      ...
      .catch(() => router.replace("/backoffice/login"));
    ...
    const crosshairPlugin = {
      ...
      afterDatasetsDraw(chart: any) {
        ...
        if (active && active.length > 0) {
          ...
          ctx.save();
          ctx.beginPath();
          ctx.moveTo(x, (yScale as any).top);
          ctx.lineTo(x, (yScale as any).bottom);
          ctx.lineWidth = 1;
  ...
  const drawFinanceCharts = () => {
    ...
    const crosshairPlugin = {
      ...
      afterDatasetsDraw(chart: any) {
        ...
        if (active && active.length > 0) {
          const ctx = chart.ctx;
          const x = active[0].element.x;
          const yScale = chart.scales.y || Object.values(chart.scales).find((s: any) => s.axis === 'y');
          if (!yScale) return;
          ctx.save();
          ctx.beginPath();
          ctx.moveTo(x, (yScale as any).top);
    ...
    if (insights?.mrrHistory && insights.mrrHistory.length > 0) {
      createChart("mrrTrendChart", {
        type: "line",
        data: {
          labels: insights.mrrHistory.map(d => d.label),
          datasets: [{
            label: "MRR", data: insights.mrrHistory.map(d => d.mrr),
            borderColor: "#009E73", backgroundColor: "rgba(0,158,115,0.08)", tension: 0.4, fill: true, borderWidth: 2, ...hoverPoint("#009E73"),
          }],
        },
        options: {
          ...baseOpts,
    ...
    createChart("funnelChart", {
      type: "bar",
      data: {
        labels: ["Signed up", "Sent 1 msg", "Sent 10 msgs", "Paid"],
        datasets: [{
          label: "Users",
          data: [computed.funnel.signupCount, computed.funnel.oneMsg, computed.funnel.tenMsg, computed.funnel.paidCount],
          backgroundColor: ["rgba(100,143,255,0.6)", "rgba(100,143,255,0.5)", "rgba(100,143,255,0.4)", "rgba(0,158,115,0.7)"],
          borderColor: ["#648FFF", "#648FFF", "#648FFF", "#009E73"],
          borderWidth: 1, borderRadius: 4,
        }],
      },
      options: { ...baseOpts, scales: { x: { grid, ticks: tickX, border: { display: false } }, y: { grid, ticks: tickY, border: { display: false }, min: 0 } } },
      plugins: [crosshairPlugin],
    });
    ...
    <div className="admin-page">
      ...
      <div className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="tbtn" onClick={handleLogout}>Sign out</button>
        </div>
      </div>
        ...
        <div className="page-content">
          {stats && (
            <div className="stat-grid">
              ...
              <div className="sc"><div className="sc-label">ACTIVE USERS</div><div className="sc-val">{hasFilter ? filteredStats.totalUsers : stats.activeUsers}</div><div className="sc-sub">sent at least 1 message</div></div>
              <div className="sc"><div className="sc-label">ENGAGED USERS</div><div className="sc-val">{stats.engagedUsers}</div><div className="sc-sub">sent at least 5 messages</div></div>
